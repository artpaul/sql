...
bool minio::s3::SelectHandler::ReadData() {
    ...
    if (response_.length() < data_length)
        return false;
...
minio::error::Error minio::s3::SelectHandler::DecodeHeader(std::map<std::string, std::string>& headers,
                                                           std::string data) {
    while (true) {
...
bool minio::s3::SelectHandler::process(http::DataFunctionArgs args, bool& cont) {
    if (!prelude_read_ && !ReadPrelude())
        return true;
    ...
    if (headers[":message-type"] == "error") {
        done_ = true;
        result_func_(SelectResult(error::Error(headers[":error-code"] + ": " + headers[":error-message"])));
        return false;
    }
    ...
    if (headers[":event-type"] == "Progress" || headers[":event-type"] == "Stats") {
        pugi::xml_document xdoc;
        pugi::xml_parse_result result = xdoc.load_string(payload.data());
        if (!result) {
            done_ = true;
            result_func_(SelectResult(error::Error("unable to parse XML; " + payload)));
            return false;
        }
        ...
        if (!value.empty())
            bytes_scanned = std::stol(value);

        text = root.node().select_node("BytesProcessed/text()");
        value = text.node().value();
        if (!value.empty())
            bytes_processed = std::stol(value);

        text = root.node().select_node("BytesReturned/text()");
        value = text.node().value();
        if (!value.empty())
            bytes_returned = std::stol(value);

        cont = result_func_(SelectResult(bytes_scanned, bytes_processed, bytes_returned));
        Reset();
        done_ = !cont;
        return cont;
    }
    ...
    done_ = true;
    result_func_(SelectResult(error::Error("unknown event-type " + headers[":event-type"])));
    return false;
}
