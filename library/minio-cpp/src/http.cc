...
minio::error::Error minio::http::Response::ReadHeaders() {
    ...
    while ((pos = lines.find("\r\n")) != std::string::npos) {
        ...
        lines.erase(0, pos + 2);
        if (error::Error err = add_header(line))
            return err;
    }

    if (!lines.empty()) {
        if (error::Error err = add_header(lines))
            return err;
    }
...
size_t minio::http::Response::ResponseCallback(
    curlpp::Multi* requests, curlpp::Easy* request, char* buffer, size_t size, size_t length) {
    ...
    if (!headers_read_) {
        ...
        if (!headers_read_ || response_.empty())
            return realsize;
...
minio::http::Response minio::http::Request::execute() {
    ...
    request.setOpt(new curlpp::options::WriteFunction(
        std::bind(&Response::ResponseCallback, &response, &requests, &request, _1, _2, _3)));

    auto progress = [&progressfunc = progressfunc, &progress_userdata = progress_userdata](
