...
minio::s3::ListObjectsResult::ListObjectsResult(Client* client, ListObjectsArgs args) {
    this->client_ = client;
    this->args_ = args;
    Populate();
}
...
minio::s3::RemoveObjectsResult::RemoveObjectsResult(Client* client, RemoveObjectsArgs args) {
    client_ = client;
    args_ = args;
    Populate();
}
...
minio::s3::StatObjectResponse minio::s3::Client::CalculatePartCount(size_t& part_count,
    ...
    for (auto& source : sources) {
        ...
        if (size < utils::kMinPartSize && sources.size() != 1 && i != sources.size()) {
            std::string msg = "source " + source.bucket + "/" + source.object;
            if (!source.version_id.empty())
                msg += "?versionId=" + source.version_id;
        ...
        if (size > utils::kMaxPartSize) {
            ...
            if (last_part_size < utils::kMinPartSize && sources.size() != 1 && i != sources.size()) {
                std::string msg = "source " + source.bucket + "/" + source.object;
                if (!source.version_id.empty()) {
                    msg += "?versionId=" + source.version_id;
                }
                msg += ": size " + std::to_string(size) + " for multipart split upload of " +
                       std::to_string(size) + ", last part size is less than " +
                       std::to_string(utils::kMinPartSize);
                return error::Error(msg);
            }
        ...
        if (part_count > utils::kMaxMultipartCount) {
            return error::Error("Compose sources create more than allowed multipart count " +
...
minio::s3::ComposeObjectResponse minio::s3::Client::ComposeObject(ComposeObjectArgs args,
    ...
    for (auto& source : args.sources) {
        ...
        if (size <= utils::kMaxPartSize) {
            part_number++;
            if (source.length != NULL) {
                headers.Add("x-amz-copy-source-range", "bytes=" + std::to_string(offset) + "-" +
            ...
            while (size > 0) {
                ...
                headerscopy.Add("x-amz-copy-source-range",
                                "bytes=" + std::to_string(start_bytes) + "-" + std::to_string(end_bytes));
...
minio::s3::PutObjectResponse minio::s3::Client::PutObject(PutObjectArgs& args,
    ...
    while (!stop) {
        ...
        if (part_count > 0) {
            ...
            if (error::Error err = utils::ReadPart(args.stream, buf, part_size, bytes_read)) {
                return err;
            }

            if (bytes_read != part_size) {
                return error::Error(
                    "not enough data in the stream; expected: " + std::to_string(part_size) +
        ...
        if (args.progressfunc != NULL) {
            up_args.progressfunc = [&object_size = object_size, &uploaded_bytes = uploaded_bytes,
                                    &upload_speed = upload_speed, &progressfunc = args.progressfunc,
                                    &progress_userdata =
                                        args.progress_userdata](http::ProgressFunctionArgs args) -> void {
...
minio::s3::ComposeObjectResponse minio::s3::Client::ComposeObject(ComposeObjectArgs args) {
    ...
    if (args.sse != NULL && args.sse->TlsRequired() && !base_url_.https) {
        return error::Error("SSE operation must be performed over a secure connection");
    }
...
minio::s3::CopyObjectResponse minio::s3::Client::CopyObject(CopyObjectArgs args) {
    if (error::Error err = args.Validate())
        return err;
    ...
    if (args.source.offset != NULL || args.source.length != NULL || size > utils::kMaxPartSize) {
        if (args.metadata_directive != NULL && *args.metadata_directive == Directive::kCopy) {
            return error::Error("COPY metadata directive is not applicable to source object size "
                                "greater than 5 GiB");
        }

        if (args.tagging_directive != NULL && *args.tagging_directive == Directive::kCopy) {
            return error::Error("COPY tagging directive is not applicable to source object size "
                                "greater than 5 GiB");
        }
    ...
    if (args.metadata_directive != NULL) {
        headers.Add("x-amz-metadata-directive", DirectiveToString(*args.metadata_directive));
    }
    if (args.tagging_directive != NULL) {
        headers.Add("x-amz-tagging-directive", DirectiveToString(*args.tagging_directive));
    }
...
minio::s3::DownloadObjectResponse minio::s3::Client::DownloadObject(DownloadObjectArgs args) {
    if (error::Error err = args.Validate())
        return err;
    ...
    std::string temp_filename = args.filename + "." + curlpp::escape(etag) + ".part.minio";
...
minio::s3::ListObjectsResult minio::s3::Client::ListObjects(ListObjectsArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::UploadObjectResponse minio::s3::Client::UploadObject(UploadObjectArgs args) {
    if (error::Error err = args.Validate())
        return err;
    ...
    try {
        ...
        return error::Error("unable to open file " + args.filename + "; " + err.code().message());
    }
...
minio::s3::RemoveObjectsResult minio::s3::Client::RemoveObjects(RemoveObjectsArgs args) {
    if (error::Error err = args.Validate())
        return err;
