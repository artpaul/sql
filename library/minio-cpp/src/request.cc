...
minio::error::Error minio::s3::getAwsInfo(std::string host,
                                          bool https,
                                          std::string& region,
                                          std::string& aws_s3_prefix,
                                          std::string& aws_domain_suffix,
                                          bool& dualstack) {
    if (!awsRegexMatch(host, HOSTNAME_REGEX))
    ...
    if (!awsRegexMatch(host, AWS_ENDPOINT_REGEX))
        return error::SUCCESS;
    ...
    if (utils::EndsWith(aws_domain_suffix, ".cn") && !utils::EndsWith(aws_s3_prefix, "s3-accelerate.") &&
        region.empty())
    {
...
minio::s3::BaseUrl::BaseUrl(std::string host, bool https, std::string region) {
    ...
    if (!url.path.empty() || !url.query_string.empty()) {
        this->err_ = error::Error("host value must contain only hostname and optional port number");
        return;
    }
    ...
    if (error::Error err = getAwsInfo(this->host, this->https, this->region, this->aws_s3_prefix,
                                      this->aws_domain_suffix, this->dualstack))
    {
        this->err_ = err;
        return;
    }
    this->virtual_style = !this->aws_domain_suffix.empty() || utils::EndsWith(this->host, "aliyuncs.com");
}
...
minio::error::Error minio::s3::BaseUrl::BuildAwsUrl(http::Url& url,
    ...
    if (host == "s3-external-1.amazonaws.com" || host == "s3-us-gov-west-1.amazonaws.com" ||
    ...
    if (utils::Contains(this->aws_s3_prefix, "s3-accelerate")) {
        ...
        if (enforce_path_style)
            host.replace(host.find("-accelerate"), 11, "");
    }
...
void minio::s3::BaseUrl::BuildListBucketsUrl(http::Url& url, std::string region) {
    ...
    if (utils::StartsWith(s3_prefix, "s3.") || utils::StartsWith(s3_prefix, "s3-")) {
        s3_prefix = "s3.";
        domain_suffix = "amazonaws.com";
        if (utils::EndsWith(this->aws_domain_suffix, ".cn"))
            domain_suffix += ".cn";
    }
...
minio::error::Error minio::s3::BaseUrl::BuildUrl(http::Url& url,
    ...
    if (!this->aws_domain_suffix.empty()) {
        if (error::Error err = this->BuildAwsUrl(url, bucket_name, enforce_path_style, region)) {
            return err;
        }
    }
...
minio::s3::Request::Request(http::Method method,
                            std::string region,
                            BaseUrl& baseurl,
...
void minio::s3::Request::BuildHeaders(http::Url& url, creds::Provider* provider) {
    ...
    if (!md5sum.empty())
        headers.Add("Content-MD5", md5sum);
    if (!sha256.empty())
    ...
    if (provider != NULL) {
        ...
        signer::SignV4S3(method, url.path, region, headers, query_params, creds.access_key,
                         creds.secret_key, sha256, date);
    }
}

minio::http::Request minio::s3::Request::ToHttpRequest(creds::Provider* provider) {
    http::Url url;
    if (error::Error err = base_url.BuildUrl(url, method, region, query_params, bucket_name, object_name)) {
