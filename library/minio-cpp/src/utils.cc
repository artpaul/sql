...
const std::string WEEK_DAYS[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
...
const std::regex OLD_BUCKET_NAME_REGEX("^[a-z0-9][a-z0-9_\\.\\-\\:]{1,61}[a-z0-9]$",
                                       std::regex_constants::icase);
const std::regex IPV4_REGEX("^((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])\\.){3}"
...
std::string minio::utils::Printable(std::string s) {
    ...
    for (auto& ch : s) {
        if (ch < 33 || ch > 126) {
            ss << "\\x" << std::hex << std::setfill('0') << std::setw(2) << (ch & 0xff);
        } else {
            ss << ch;
        }
    }
...
bool minio::utils::StringToBool(std::string str) {
    ...
    std::cerr << "ABORT: Unknown bool string. This should not happen." << std::endl;
    std::terminate();
...
std::string minio::utils::Join(std::list<std::string> values, std::string delimiter) {
    std::string result;
...
std::string minio::utils::Join(std::vector<std::string> values, std::string delimiter) {
    std::string result;
    for (const auto& value : values) {
        if (!result.empty())
            result += delimiter;
...
std::string minio::utils::Time::ToHttpHeaderValue() {
    std::tm* utc = ToUTC();
    std::stringstream ss;
    ss << WEEK_DAYS[utc->tm_wday] << ", " << FormatTime(utc, "%d ") << MONTHS[utc->tm_mon]
       << FormatTime(utc, " %Y %H:%M:%S GMT");
    return ss.str();
}
...
minio::utils::Time minio::utils::Time::FromHttpHeaderValue(const char* value) {
    ...
    // Parse week day.
    auto pos = std::find(std::begin(WEEK_DAYS), std::end(WEEK_DAYS), s.substr(0, 3));
...
void minio::utils::Multimap::GetCanonicalHeaders(std::string& signed_headers,
...
minio::error::Error minio::utils::CheckBucketName(std::string_view bucket_name, bool strict) {
    ...
    if (Contains(bucket_name, "..") || Contains(bucket_name, ".-") || Contains(bucket_name, "-.")) {
        return error::Error("Bucket name contains invalid successive characters '..', '.-' or "
                            "'-.'");
    }
...
minio::error::Error minio::utils::ReadPart(std::istream& stream,
                                           char* buf,
                                           size_t size,
                                           size_t& bytes_read) {
...
minio::error::Error minio::utils::CalcPartInfo(long object_size, size_t& part_size, long& part_count) {
    if (part_size > 0) {
    ...
    if (object_size >= 0) {
        ...
        return error::Error("valid part size must be provided when object size is unknown");
    }
    ...
    part_count = part_size > 0 ? (long)std::ceil((double)object_size / part_size) : 1;
    if (part_count > kMaxMultipartCount) {
        return error::Error("object size " + std::to_string(object_size) + " and part size " +
