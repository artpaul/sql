...
minio::s3::ListBucketsResponse minio::s3::ListBucketsResponse::ParseXML(std::string_view data) {
    ...
    pugi::xpath_node_set xnodes = xdoc.select_nodes("/ListAllMyBucketsResult/Buckets/Bucket");
...
minio::s3::ListObjectsResponse minio::s3::ListObjectsResponse::ParseXML(std::string_view data,
                                                                        bool version) {
    ...
    value = text.node().value();
    if (!value.empty())
        resp.is_truncated = utils::StringToBool(value);

    text = root.node().select_node("MaxKeys/text()");
    value = text.node().value();
    if (!value.empty())
        resp.max_keys = std::stoi(value);

    // ListBucketResult V1
    {
        text = root.node().select_node("Marker/text()");
        value = text.node().value();
        resp.marker = (resp.encoding_type == "url") ? curlpp::unescape(value) : value;

        text = root.node().select_node("NextMarker/text()");
        value = text.node().value();
        resp.next_marker = (resp.encoding_type == "url") ? curlpp::unescape(value) : value;
    }
    ...
    // ListBucketResult V2
    {
        ...
        value = text.node().value();
        if (!value.empty())
            resp.key_count = std::stoi(value);

        text = root.node().select_node("StartAfter/text()");
        value = text.node().value();
        resp.start_after = (resp.encoding_type == "url") ? curlpp::unescape(value) : value;
    ...
    // ListVersionsResult
    {
        ...
        value = text.node().value();
        resp.key_marker = (resp.encoding_type == "url") ? curlpp::unescape(value) : value;

        text = root.node().select_node("NextKeyMarker/text()");
        value = text.node().value();
        resp.next_key_marker = (resp.encoding_type == "url") ? curlpp::unescape(value) : value;
    ...
    auto populate = [&resp = resp, &last_item = last_item](std::list<Item>& items,
                                                           pugi::xpath_node_set& contents,
        ...
        for (auto content : contents) {
            ...
            value = text.node().value();
            if (!value.empty())
                item.is_latest = utils::StringToBool(value);
...
minio::s3::RemoveObjectsResponse minio::s3::RemoveObjectsResponse::ParseXML(std::string_view data) {
    ...
    for (auto deleted : deletedList) {
        ...
        value = text.node().value();
        if (!value.empty())
            object.delete_marker = utils::StringToBool(value);
...
minio::s3::GetBucketNotificationResponse minio::s3::GetBucketNotificationResponse::ParseXML(
    ...
    auto populate = [](pugi::xpath_node& common) -> NotificationCommonConfig {
        ...
        for (auto event : events)
            cfg.events.push_back(event.node().value());
...
minio::s3::GetBucketEncryptionResponse minio::s3::GetBucketEncryptionResponse::ParseXML(
    std::string_view data) {
    SseConfig config;
    ...
    auto root = xdoc.select_node("/ServerSideEncryptionConfiguration/Rule"
...
minio::s3::GetBucketReplicationResponse minio::s3::GetBucketReplicationResponse::ParseXML(
    std::string_view data) {
    ...
    for (auto& rule : rules) {
        ...
        if (destination.node().select_node("AccessControlTranslation")) {
            ...
            text = destination.node().select_node("AccessControlTranslation/Owner/text()");
        ...
        if (destination.node().select_node("EncryptionConfiguration")) {
            ...
            text = destination.node().select_node("EncryptionConfiguration/ReplicaKmsKeyID/text()");
            rrule.destination.encryption_config.replica_kms_key_id = text.node().value();
        }
        if (destination.node().select_node("Metrics")) {
            rrule.destination.metrics.Enable();

            text = destination.node().select_node("Metrics/EventThreshold/Minutes/text()");
            value = text.node().value();
            rrule.destination.metrics.event_threshold_minutes = std::stoi(value);

            text = destination.node().select_node("Metrics/EventThreshold/Status/text()");
            value = text.node().value();
        ...
        if (rule.node().select_node("SourceSelectionCriteria")) {
            ...
            if (rule.node().select_node("SourceSelectionCriteria/SseKmsEncryptedObjects")) {
                text =
                    rule.node().select_node("SourceSelectionCriteria/SseKmsEncryptedObjects/Status/text()");
                value = text.node().value();
                rrule.source_selection_criteria.sse_kms_encrypted_objects_status = (value == "Enabled");
            }
        }
...
minio::s3::GetBucketLifecycleResponse minio::s3::GetBucketLifecycleResponse::ParseXML(
    std::string_view data) {
    ...
    for (auto& rule : rules) {
        ...
        if (rule.node().select_node("AbortIncompleteMultipartUpload")) {
            text = rule.node().select_node("AbortIncompleteMultipartUpload/DaysAfterInitiation/text()");
            value = text.node().value();
            lrule.abort_incomplete_multipart_upload_days_after_initiation = std::stoi(value);
        }
        ...
        if (rule.node().select_node("Expiration")) {
            if (rule.node().select_node("Expiration/Date")) {
                ...
                lrule.expiration_date = utils::Time::FromISO8601UTC(text.node().value());
            }
            ...
            if (rule.node().select_node("Expiration/ExpiredObjectDeleteMarker")) {
                text = rule.node().select_node("Expiration/ExpiredObjectDeleteMarker/text()");
                lrule.expiration_expired_object_delete_marker = utils::BoolToString(text.node().value());
            }
        }
        ...
        if (rule.node().select_node("NoncurrentVersionExpiration")) {
            text = rule.node().select_node("NoncurrentVersionExpiration/NoncurrentDays/text()");
            value = text.node().value();
            lrule.noncurrent_version_expiration_noncurrent_days = std::stoi(value);
        }

        if (rule.node().select_node("NoncurrentVersionTransition")) {
            text = rule.node().select_node("NoncurrentVersionTransition/NoncurrentDays/text()");
            value = text.node().value();
            lrule.noncurrent_version_transition_noncurrent_days = std::stoi(value);

            text = rule.node().select_node("NoncurrentVersionTransition/StorageClass/text()");
        ...
        if (rule.node().select_node("Transition")) {
            if (rule.node().select_node("Transition/Date")) {
                ...
                lrule.transition_date = utils::Time::FromISO8601UTC(text.node().value());
            }
...
minio::s3::GetBucketTagsResponse minio::s3::GetBucketTagsResponse::ParseXML(std::string_view data) {
...
minio::s3::GetObjectTagsResponse minio::s3::GetObjectTagsResponse::ParseXML(std::string_view data) {
    std::map<std::string, std::string> map;
