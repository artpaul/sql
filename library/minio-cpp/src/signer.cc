...
std::string minio::signer::GetScope(utils::Time& time, std::string& region, std::string& service_name) {
    return time.ToSignerDate() + "/" + region + "/" + service_name + "/aws4_request";
}

std::string minio::signer::GetCanonicalRequestHash(std::string& method,
                                                   std::string& uri,
...
std::string minio::signer::GetStringToSign(utils::Time& date,
                                           std::string& scope,
                                           std::string& canonical_request_hash) {
    return "AWS4-HMAC-SHA256\n" + date.ToAmzDate() + "\n" + scope + "\n" + canonical_request_hash;
}

std::string minio::signer::HmacHash(std::string_view key, std::string_view data) {
    std::array<unsigned char, EVP_MAX_MD_SIZE> hash;
    unsigned int hash_len;

    HMAC(EVP_sha256(), key.data(), static_cast<int>(key.size()),
         reinterpret_cast<const unsigned char*>(data.data()), static_cast<int>(data.size()), hash.data(),
         &hash_len);

    return std::string{reinterpret_cast<const char*>(hash.data()), hash_len};
}
...
std::string minio::signer::GetSignature(std::string_view signing_key, std::string_view string_to_sign) {
...
minio::utils::Multimap& minio::signer::SignV4(std::string& service_name,
                                              http::Method& method,
                                              std::string& uri,
                                              std::string& region,
    ...
    std::string canonical_request_hash = GetCanonicalRequestHash(
        methodstring, uri, canonical_query_string, canonical_headers, signed_headers, content_sha256);

    std::string string_to_sign = GetStringToSign(date, scope, canonical_request_hash);

    std::string signing_key = GetSigningKey(secret_key, date, region, service_name);

    std::string signature = GetSignature(signing_key, string_to_sign);

    std::string authorization = GetAuthorization(access_key, scope, signed_headers, signature);

    headers.Add("Authorization", authorization);
    return headers;
}

minio::utils::Multimap& minio::signer::SignV4S3(http::Method method,
                                                std::string& uri,
                                                std::string& region,
                                                utils::Multimap& headers,
                                                utils::Multimap query_params,
    ...
    return SignV4(service_name, method, uri, region, headers, query_params, access_key, secret_key,
                  content_sha256, date);
}

minio::utils::Multimap& minio::signer::SignV4STS(http::Method method,
                                                 std::string& uri,
                                                 std::string& region,
                                                 utils::Multimap& headers,
                                                 utils::Multimap query_params,
...
minio::utils::Multimap& minio::signer::PresignV4(http::Method method,
                                                 std::string& host,
                                                 std::string& uri,
                                                 std::string& region,
                                                 utils::Multimap& query_params,
