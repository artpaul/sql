...
minio::utils::Multimap minio::s3::GetCommonListObjectsQueryParams(std::string& delimiter,
    ...
    if (!encoding_type.empty())
        query_params.Add("encoding-type", encoding_type);
    return query_params;
}
...
minio::s3::BaseClient::BaseClient(BaseUrl& base_url, creds::Provider* provider)
    : base_url_(base_url) {
    if (!base_url_) {
        std::cerr << "valid base url must be provided; " << base_url_.Error().String() << std::endl;
        std::terminate();
    }
...
minio::error::Error minio::s3::BaseClient::SetAppInfo(std::string_view app_name,
    ...
    user_agent_ =
        std::string(DEFAULT_USER_AGENT) + " " + std::string(app_name) + "/" + std::string(app_version);
    return error::SUCCESS;
}

void minio::s3::BaseClient::HandleRedirectResponse(std::string& code,
                                                   std::string& message,
                                                   int status_code,
                                                   http::Method method,
    ...
    if (retry && !region.empty() && method == http::Method::kHead && !bucket_name.empty() &&
        !region_map_[bucket_name].empty())
    {
        code = "RetryHead";
        message = "";
    }
...
minio::s3::Response minio::s3::BaseClient::GetErrorResponse(http::Response resp,
    ...
    if (!resp.body.empty()) {
        ...
        Response response(
            error::Error("invalid response received; status code: " + std::to_string(resp.status_code) +
    ...
    switch (resp.status_code) {
        ...
        case 307:
        case 400:
            HandleRedirectResponse(response.code, response.message, resp.status_code, method, resp.headers,
                                   bucket_name, true);
            break;
        case 403:
        ...
        case 405:
            ...
            response.message = "The specified method is not allowed against this resource";
            break;
        case 409:
        ...
        default:
            Response response(
                error::Error("server failed with HTTP status code " + std::to_string(resp.status_code)));
            response.status_code = resp.status_code;
            response.headers = resp.headers;
            return response;
    }
...
minio::s3::Response minio::s3::BaseClient::execute(Request& req) {
    ...
    Response resp =
        GetErrorResponse(response, request.url.path, req.method, req.bucket_name, req.object_name);
...
minio::s3::Response minio::s3::BaseClient::Execute(Request& req) {
    ...
    std::string code;
    std::string message;
    HandleRedirectResponse(code, message, resp.status_code, req.method, resp.headers, req.bucket_name);
    resp.code = code;
...
minio::s3::GetRegionResponse minio::s3::BaseClient::GetRegion(std::string& bucket_name,
                                                              std::string& region) {
    std::string base_region = base_url_.region;
    if (!region.empty()) {
        if (!base_region.empty() && base_region != region) {
            return error::Error("region must be " + base_region + ", but passed " + region);
        }
    ...
    if (bucket_name.empty() || provider_ == NULL)
    ...
    Request req(http::Method::kGet, "us-east-1", base_url_, utils::Multimap(), utils::Multimap());
    ...
    if (!result)
        return error::Error("unable to parse XML");
...
minio::s3::AbortMultipartUploadResponse minio::s3::BaseClient::AbortMultipartUpload(
    ...
    Request req(http::Method::kDelete, region, base_url_, args.extra_headers, args.extra_query_params);
...
minio::s3::BucketExistsResponse minio::s3::BaseClient::BucketExists(BucketExistsArgs args) {
    if (error::Error err = args.Validate())
        return err;
    ...
    Request req(http::Method::kHead, region, base_url_, args.extra_headers, args.extra_query_params);
    req.bucket_name = args.bucket;
...
minio::s3::CompleteMultipartUploadResponse minio::s3::BaseClient::CompleteMultipartUpload(
    ...
    Request req(http::Method::kPost, region, base_url_, args.extra_headers, args.extra_query_params);
    req.bucket_name = args.bucket;
    req.object_name = args.object;
    req.query_params.Add("uploadId", args.upload_id);
    ...
    if (!response)
        return response;
    return CompleteMultipartUploadResponse::ParseXML(response.data,
                                                     response.headers.GetFront("x-amz-version-id"));
}

minio::s3::CreateMultipartUploadResponse minio::s3::BaseClient::CreateMultipartUpload(
    ...
    if (Response resp = Execute(req)) {
        ...
        if (!result)
            return error::Error("unable to parse XML");
        auto text = xdoc.select_node("/InitiateMultipartUploadResult/UploadId/text()");
...
minio::s3::DeleteBucketEncryptionResponse minio::s3::BaseClient::DeleteBucketEncryption(
...
minio::s3::DisableObjectLegalHoldResponse minio::s3::BaseClient::DisableObjectLegalHold(
    ...
    Request req(http::Method::kPut, region, base_url_, args.extra_headers, args.extra_query_params);
    req.bucket_name = args.bucket;
    req.object_name = args.object;
...
minio::s3::DeleteBucketLifecycleResponse minio::s3::BaseClient::DeleteBucketLifecycle(
...
minio::s3::DeleteBucketNotificationResponse minio::s3::BaseClient::DeleteBucketNotification(
...
minio::s3::DeleteBucketReplicationResponse minio::s3::BaseClient::DeleteBucketReplication(
...
minio::s3::DeleteBucketTagsResponse minio::s3::BaseClient::DeleteBucketTags(DeleteBucketTagsArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::DeleteObjectLockConfigResponse minio::s3::BaseClient::DeleteObjectLockConfig(
...
minio::s3::DeleteObjectTagsResponse minio::s3::BaseClient::DeleteObjectTags(DeleteObjectTagsArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::EnableObjectLegalHoldResponse minio::s3::BaseClient::EnableObjectLegalHold(
...
minio::s3::GetBucketEncryptionResponse minio::s3::BaseClient::GetBucketEncryption(
    ...
    Request req(http::Method::kGet, region, base_url_, args.extra_headers, args.extra_query_params);
    req.bucket_name = args.bucket;
    req.query_params.Add("encryption", "");
...
minio::s3::GetBucketNotificationResponse minio::s3::BaseClient::GetBucketNotification(
...
minio::s3::GetBucketPolicyResponse minio::s3::BaseClient::GetBucketPolicy(GetBucketPolicyArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::GetBucketReplicationResponse minio::s3::BaseClient::GetBucketReplication(
...
minio::s3::GetBucketTagsResponse minio::s3::BaseClient::GetBucketTags(GetBucketTagsArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::GetBucketVersioningResponse minio::s3::BaseClient::GetBucketVersioning(
...
minio::s3::GetObjectResponse minio::s3::BaseClient::GetObject(GetObjectArgs args) {
    if (error::Error err = args.Validate())
        return err;

    if (args.ssec != NULL && !base_url_.https) {
        return error::Error("SSE-C operation must be performed over a secure connection");
    }
    ...
    if (args.ssec != NULL)
        req.headers.AddAll(args.ssec->Headers());

    return Execute(req);
}

minio::s3::GetObjectLockConfigResponse minio::s3::BaseClient::GetObjectLockConfig(
...
minio::s3::GetObjectTagsResponse minio::s3::BaseClient::GetObjectTags(GetObjectTagsArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::GetPresignedObjectUrlResponse minio::s3::BaseClient::GetPresignedObjectUrl(
    ...
    http::Url url;
    if (error::Error err =
            base_url_.BuildUrl(url, args.method, region, query_params, args.bucket, args.object)) {
        std::cerr << "failed to build url. error=" << err << ". This should not happen" << std::endl;
        std::terminate();
    }
    ...
    if (provider_ != NULL) {
        ...
        if (args.request_time)
            date = args.request_time;

        signer::PresignV4(args.method, url.host, url.path, region, query_params, creds.access_key,
                          creds.secret_key, date, args.expiry_seconds);
...
minio::s3::GetPresignedPostFormDataResponse minio::s3::BaseClient::GetPresignedPostFormData(
    PostPolicy policy) {
    if (!policy) {
    ...
    if (provider_ == NULL) {
        return error::Error("Anonymous access does not require presigned post form-data");
    }
    ...
    if (error::Error err =
            policy.FormData(data, creds.access_key, creds.secret_key, creds.session_token, region))
    {
        return err;
    }
    return data;
...
minio::s3::IsObjectLegalHoldEnabledResponse minio::s3::BaseClient::IsObjectLegalHoldEnabled(
...
minio::s3::ListBucketsResponse minio::s3::BaseClient::ListBuckets(ListBucketsArgs args) {
    Request req(http::Method::kGet, base_url_.region, base_url_, args.extra_headers,
...
minio::s3::ListenBucketNotificationResponse minio::s3::BaseClient::ListenBucketNotification(
    ListenBucketNotificationArgs args) {
    if (error::Error err = args.Validate())
        return err;

    if (!base_url_.aws_domain_suffix.empty()) {
        return error::Error("ListenBucketNotification API is not supported in Amazon S3");
    }
    ...
    Request req =
        Request(http::Method::kGet, region, base_url_, args.extra_headers, args.extra_query_params);
    req.bucket_name = args.bucket;
    ...
    if (args.events.size() > 0) {
        for (auto& event : args.events)
            req.query_params.Add("events", event);
    } else {
    ...
    std::string data;
    auto func = args.func;
    req.datafunc = [&func = func, &data = data](http::DataFunctionArgs args) -> bool {
        data += args.datachunk;
        while (true) {
...
minio::s3::ListObjectsResponse minio::s3::BaseClient::ListObjectsV1(ListObjectsV1Args args) {
    if (error::Error err = args.Validate())
        return err;
    ...
    Request req(http::Method::kGet, region, base_url_, args.extra_headers, args.extra_query_params);
    req.bucket_name = args.bucket;
    req.query_params.AddAll(
        GetCommonListObjectsQueryParams(args.delimiter, args.encoding_type, args.max_keys, args.prefix));
...
minio::s3::ListObjectsResponse minio::s3::BaseClient::ListObjectsV2(ListObjectsV2Args args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::ListObjectsResponse minio::s3::BaseClient::ListObjectVersions(ListObjectVersionsArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::MakeBucketResponse minio::s3::BaseClient::MakeBucket(MakeBucketArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::PutObjectResponse minio::s3::BaseClient::PutObject(PutObjectApiArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::RemoveBucketResponse minio::s3::BaseClient::RemoveBucket(RemoveBucketArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::RemoveObjectResponse minio::s3::BaseClient::RemoveObject(RemoveObjectArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::RemoveObjectsResponse minio::s3::BaseClient::RemoveObjects(RemoveObjectsApiArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::SelectObjectContentResponse minio::s3::BaseClient::SelectObjectContent(
...
minio::s3::SetBucketEncryptionResponse minio::s3::BaseClient::SetBucketEncryption(
    ...
    if (!args.config.kms_master_key_id.empty()) {
        ss << "<KMSMasterKeyID>" << args.config.kms_master_key_id << "</KMSMasterKeyID>";
    }
...
minio::s3::SetBucketNotificationResponse minio::s3::BaseClient::SetBucketNotification(
...
minio::s3::SetBucketPolicyResponse minio::s3::BaseClient::SetBucketPolicy(SetBucketPolicyArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::SetBucketReplicationResponse minio::s3::BaseClient::SetBucketReplication(
...
minio::s3::SetBucketTagsResponse minio::s3::BaseClient::SetBucketTags(SetBucketTagsArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::SetBucketVersioningResponse minio::s3::BaseClient::SetBucketVersioning(
    ...
    if (args.status) {
        ss << "<Status>" << (args.status.Get() ? "Enabled" : "Suspended") << "</Status>";
    }
    if (args.mfa_delete) {
        ss << "<MFADelete>" << (args.mfa_delete.Get() ? "Enabled" : "Disabled") << "</MFADelete>";
    }
    ss << "</VersioningConfiguration>";
    std::string body = ss.str();
...
minio::s3::SetObjectLockConfigResponse minio::s3::BaseClient::SetObjectLockConfig(
    ...
    if (IsRetentionModeValid(args.config.retention_mode)) {
        ...
        ss << "<Mode>" << RetentionModeToString(args.config.retention_mode) << "</Mode>";
        if (args.config.retention_duration_days) {
            ss << "<Days>" << std::to_string(args.config.retention_duration_days.Get()) << "</Days>";
        }
        if (args.config.retention_duration_years) {
            ss << "<Years>" << std::to_string(args.config.retention_duration_years.Get()) << "</Years>";
        }
...
minio::s3::SetObjectRetentionResponse minio::s3::BaseClient::SetObjectRetention(
    ...
    ss << "<Retention>"
       ...
       << "<RetainUntilDate>" << args.retain_until_date.ToISO8601UTC() << "</RetainUntilDate>"
       << "</Retention>";
...
minio::s3::SetObjectTagsResponse minio::s3::BaseClient::SetObjectTags(SetObjectTagsArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::StatObjectResponse minio::s3::BaseClient::StatObject(StatObjectArgs args) {
    if (error::Error err = args.Validate())
        return err;
    ...
    if (!value.empty())
        resp.retention_mode = StringToRetentionMode(value);

    value = response.headers.GetFront("x-amz-object-lock-retain-until-date");
    if (!value.empty()) {
        resp.retention_retain_until_date = utils::Time::FromISO8601UTC(value.c_str());
    }

    value = response.headers.GetFront("x-amz-object-lock-legal-hold");
    if (!value.empty())
        resp.legal_hold = StringToLegalHold(value);

    value = response.headers.GetFront("x-amz-delete-marker");
    if (!value.empty())
        resp.delete_marker = utils::StringToBool(value);
...
minio::s3::UploadPartResponse minio::s3::BaseClient::UploadPart(UploadPartArgs args) {
    if (error::Error err = args.Validate())
        return err;
...
minio::s3::UploadPartCopyResponse minio::s3::BaseClient::UploadPartCopy(UploadPartCopyArgs args) {
    if (error::Error err = args.Validate())
        return err;
