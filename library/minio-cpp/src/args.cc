...
minio::utils::Multimap minio::s3::ObjectWriteArgs::Headers() {
    ...
    if (retention != NULL) {
        headers.Add("x-amz-object-lock-mode", RetentionModeToString(retention->mode));
        headers.Add("x-amz-object-lock-retain-until-date", retention->retain_until_date.ToISO8601UTC());
    }
...
minio::utils::Multimap minio::s3::ObjectConditionalReadArgs::Headers() {
    ...
    if (!match_etag.empty())
        headers.Add("if-match", match_etag);
    if (!not_match_etag.empty())
        headers.Add("if-none-match", not_match_etag);
    if (modified_since) {
...
minio::utils::Multimap minio::s3::ObjectConditionalReadArgs::CopyHeaders() {
    ...
    if (ssec != NULL)
        headers.AddAll(ssec->CopyHeaders());
    if (!match_etag.empty()) {
    ...
    if (modified_since) {
        headers.Add("x-amz-copy-source-if-modified-since", modified_since.ToHttpHeaderValue());
    }
    if (unmodified_since) {
        headers.Add("x-amz-copy-source-if-unmodified-since", unmodified_since.ToHttpHeaderValue());
    }
...
minio::s3::ListObjectVersionsArgs::ListObjectVersionsArgs(ListObjectsArgs args) {
    this->extra_headers = args.extra_headers;
    this->extra_query_params = args.extra_query_params;
    this->bucket = args.bucket;
    this->region = args.region;
...
minio::s3::PutObjectArgs::PutObjectArgs(std::istream& istream, long object_size, long part_size)
    : stream(istream) {
...
minio::error::Error minio::s3::CopyObjectArgs::Validate() {
    ...
    if (source.offset != NULL || source.length != NULL) {
        if (metadata_directive != NULL && *metadata_directive == Directive::kCopy) {
            return error::Error("COPY metadata directive is not applicable to source object with "
                                "range");
        }

        if (tagging_directive != NULL && *tagging_directive == Directive::kCopy) {
            return error::Error("COPY tagging directive is not applicable to source object with "
                                "range");
        }
    }
...
minio::error::Error minio::s3::ComposeSource::BuildHeaders(size_t object_size, std::string& etag) {
    ...
    if (offset != NULL && *offset >= object_size) {
        return error::Error(msg + "offset " + std::to_string(*offset) + " is beyond object size " +
                            std::to_string(object_size));
    }

    if (length != NULL) {
        if (*length > object_size) {
            return error::Error(msg + "length " + std::to_string(*length) + " is beyond object size " +
                                std::to_string(object_size));
        }
        ...
        if ((off + *length) > object_size) {
            return error::Error(msg + "compose size " + std::to_string(off + *length) +
...
minio::error::Error minio::s3::SelectObjectContentArgs::Validate() {
    ...
    if (!((request.csv_input != NULL) ^ (request.json_input != NULL) ^ (request.parquet_input != NULL))) {
        return error::Error("One of CSV, JSON or Parquet input serialization must be set");
    }
...
minio::error::Error minio::s3::SetBucketTagsArgs::Validate() {
    ...
    if (tags.size() > 50) {
        return error::Error("too many bucket tags; allowed = 50, found = " + std::to_string(tags.size()));
    }
...
minio::error::Error minio::s3::SetObjectTagsArgs::Validate() {
    ...
    if (tags.size() > 10) {
        return error::Error("too many object tags; allowed = 10, found = " + std::to_string(tags.size()));
    }
