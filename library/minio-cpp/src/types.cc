...
minio::s3::RetentionMode minio::s3::StringToRetentionMode(std::string_view str) throw() {
    if (str == "GOVERNANCE")
        return RetentionMode::kGovernance;
    if (str == "COMPLIANCE")
        return RetentionMode::kCompliance;

    std::cerr << "ABORT: Unknown retention mode. This should not happen." << std::endl;
    std::terminate();

    return RetentionMode::kGovernance; // never reaches here.
}

minio::s3::LegalHold minio::s3::StringToLegalHold(std::string_view str) throw() {
    if (str == "ON")
        return LegalHold::kOn;
    if (str == "OFF")
        return LegalHold::kOff;

    std::cerr << "ABORT: Unknown legal hold. This should not happen." << std::endl;
    std::terminate();

    return LegalHold::kOff; // never reaches here.
}

minio::s3::Directive minio::s3::StringToDirective(std::string_view str) throw() {
    if (str == "COPY")
        return Directive::kCopy;
    if (str == "REPLACE")
        return Directive::kReplace;
...
std::string minio::s3::SelectRequest::ToXML() {
    ...
    if (csv_input != NULL) {
        if (csv_input->compression_type != NULL) {
            ss << "<CompressionType>" << CompressionTypeToString(*csv_input->compression_type)
               << "</CompressionType>";
        }
        ...
        if (csv_input->field_delimiter) {
            ss << "<FieldDelimiter>" << csv_input->field_delimiter << "</FieldDelimiter>";
        }
        if (csv_input->file_header_info != NULL) {
            ss << "<FileHeaderInfo>" << FileHeaderInfoToString(*csv_input->file_header_info)
               << "</FileHeaderInfo>";
        }
        if (csv_input->quote_character) {
            ss << "<QuoteCharacter>" << csv_input->quote_character << "</QuoteCharacter>";
        }
        if (csv_input->record_delimiter) {
            ss << "<RecordDelimiter>" << csv_input->record_delimiter << "</RecordDelimiter>";
        }
        ss << "</CSV>";
    }
    ...
    if (json_input != NULL) {
        if (json_input->compression_type != NULL) {
            ss << "<CompressionType>" << CompressionTypeToString(*json_input->compression_type)
               << "</CompressionType>";
        }
    ...
    if (csv_output != NULL) {
        ...
        if (csv_output->field_delimiter) {
            ss << "<FieldDelimiter>" << csv_output->field_delimiter << "</FieldDelimiter>";
        }
        if (csv_output->quote_character) {
            ss << "<QuoteCharacter>" << csv_output->quote_character << "</QuoteCharacter>";
        }
        ...
        if (csv_output->quote_fields != NULL) {
            ss << "<QuoteFields>" << QuoteFieldsToString(*csv_output->quote_fields) << "</QuoteFields>";
        }
        if (csv_output->record_delimiter) {
            ss << "<RecordDelimiter>" << csv_output->record_delimiter << "</RecordDelimiter>";
        }
        ss << "</CSV>";
    }
    ...
    if (json_output != NULL) {
        ...
        if (json_output->record_delimiter) {
            ss << "<RecordDelimiter>" << json_output->record_delimiter << "</RecordDelimiter>";
        }
        ss << "</JSON>";
    }
...
minio::s3::NotificationRecord minio::s3::NotificationRecord::ParseJSON(nlohmann::json j_record) {
    ...
    if (j_record.contains("userIdentity")) {
        record.user_identity.principal_id = j_record["userIdentity"].value("principalId", "");
    }
    if (j_record.contains("requestParameters")) {
        ...
        record.request_parameters.source_ip_address = j.value("sourceIPAddress", "");
    }
    if (j_record.contains("responseElements")) {
        ...
        record.response_elements.x_minio_deployment_id = j.value("x-minio-deployment-id", "");
        record.response_elements.x_minio_origin_endpoint = j.value("x-minio-origin-endpoint", "");
    }
    ...
    for (auto& config : cloud_func_config_list) {
        ...
        if (config.prefix_filter_rule || config.suffix_filter_rule) {
            ...
            if (config.prefix_filter_rule) {
                ...
                ss << "<Value>" << config.prefix_filter_rule.Value() << "</Value></FilterRule>";
            }
            if (config.suffix_filter_rule) {
                ss << "<FilterRule><Name>suffix</Name>";
                ss << "<Value>" << config.suffix_filter_rule.Value() << "</Value></FilterRule>";
            }
            ss << "</S3Key></Filter>";
        }
        ss << "</CloudFunctionConfiguration>";
...
std::string minio::s3::ReplicationConfig::ToXML() {
    ...
    for (auto& rule : rules) {
        ...
        if (rule.destination.access_control_translation) {
            ss << "<AccessControlTranslation><Owner>" << rule.destination.access_control_translation.owner
        ...
        if (rule.destination.encryption_config) {
            ...
            if (!rule.destination.encryption_config.replica_kms_key_id.empty()) {
                ss << "<ReplicaKmsKeyID>" << rule.destination.encryption_config.replica_kms_key_id
                   << "</ReplicaKmsKeyID>";
            }
            ss << "</EncryptionConfiguration>";
        }
        ...
        if (rule.destination.replication_time) {
            ...
            ss << "</Time>" << status_xml(rule.destination.replication_time.status) << "</ReplicationTime>";
        }
        if (!rule.destination.storage_class.empty()) {
            ss << "<StorageClass>" << rule.destination.storage_class << "</StorageClass>";
        }
        ss << "</Destination>";

        if (rule.delete_marker_replication_status) {
            ss << "<DeleteMarkerReplication>" << status_xml(rule.delete_marker_replication_status.Get())
               << "</DeleteMarkerReplication>";
        }

        if (rule.existing_object_replication_status) {
            ss << "<ExistingObjectReplication>" << status_xml(rule.existing_object_replication_status.Get())
        ...
        if (rule.source_selection_criteria) {
            ...
            if (rule.source_selection_criteria.sse_kms_encrypted_objects_status) {
                ss << "<SseKmsEncryptedObjects>"
                   << status_xml(rule.source_selection_criteria.sse_kms_encrypted_objects_status.Get())
        ...
        if (rule.delete_replication_status) {
            ss << "<DeleteReplication>" << status_xml(rule.delete_replication_status.Get())
               << "</DeleteReplication>";
        }
...
minio::error::Error minio::s3::LifecycleRule::Validate() {
    if (!abort_incomplete_multipart_upload_days_after_initiation && !expiration_date && !expiration_days &&
        !expiration_expired_object_delete_marker && !noncurrent_version_expiration_noncurrent_days &&
        noncurrent_version_transition_storage_class.empty() && !transition_date && !transition_days &&
        transition_storage_class.empty())
    {
        return error::Error("at least one of action (AbortIncompleteMultipartUpload, Expiration, "
    ...
    if (expiration_expired_object_delete_marker) {
        if (expiration_date || expiration_days) {
            return error::Error("ExpiredObjectDeleteMarker must not be provided along with Date and "
                                "Days");
        }
    ...
    for (auto& rule : rules) {
        ...
        if (rule.abort_incomplete_multipart_upload_days_after_initiation) {
            ss << "<AbortIncompleteMultipartUpload><DaysAfterInitiation>"
               << std::to_string(rule.abort_incomplete_multipart_upload_days_after_initiation)
               << "</DaysAfterInitiation></AbortIncompleteMultipartUpload>";
        }

        if (rule.expiration_date || rule.expiration_days || rule.expiration_expired_object_delete_marker) {
            ss << "<Expiration>";
            ...
            !rule.noncurrent_version_transition_storage_class.empty())
        {
            ...
            if (rule.noncurrent_version_transition_noncurrent_days) {
                ss << "<NoncurrentDays>" << rule.noncurrent_version_expiration_noncurrent_days
                   << "</NoncurrentDays>";
            }
            if (!rule.noncurrent_version_transition_storage_class.empty()) {
                ss << "<StorageClass>" << rule.noncurrent_version_transition_storage_class
                   << "</StorageClass>";
            }
            ss << "</NoncurrentVersionTransition>";
        }
        ...
        if (rule.transition_date || rule.transition_days || !rule.transition_storage_class.empty()) {
            ss << "<Transition>";
            ...
            if (!rule.transition_storage_class.empty()) {
                ss << "<StorageClass>" << rule.transition_storage_class << "</StorageClass>";
            }
            ss << "</Transition>";
        }
...
minio::error::Error minio::s3::ObjectLockConfig::Validate() {
    if (IsRetentionModeValid(retention_mode)) {
        if (!(retention_duration_days ^ retention_duration_years)) {
            return error::Error("retention mode must be provided with retention duration days or "
                                "years");
        }
    } else if (retention_duration_days || retention_duration_years) {
        return error::Error("retention duration days or years must be provided with retention "
                            "mode");
    }
