...
inline constexpr unsigned long kMaxObjectSize = 5L * 1024 * 1024 * 1024 * 1024; // 5TiB
...
error::Error ReadPart(std::istream& stream, char* buf, size_t size, size_t& bytes_read);
error::Error CalcPartInfo(long object_size, size_t& part_size, long& part_count);
