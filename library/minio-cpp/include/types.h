...
constexpr const char* CompressionTypeToString(CompressionType& ctype) throw() {
    switch (ctype) {
        ...
        case CompressionType::kBZip2:
            return "BZIP2";
        default: {
            std::cerr << "ABORT: Unknown compression type. This should not happen." << std::endl;
            std::terminate();
        }
    }
    return NULL;
...
constexpr const char* FileHeaderInfoToString(FileHeaderInfo& info) throw() {
    switch (info) {
        ...
        case FileHeaderInfo::kNone:
            return "NONE";
        default: {
            std::cerr << "ABORT: Unknown file header info. This should not happen." << std::endl;
            std::terminate();
        }
    }
    return NULL;
...
constexpr const char* JsonTypeToString(JsonType& jtype) throw() {
    switch (jtype) {
        ...
        case JsonType::kLines:
            return "LINES";
        default: {
            std::cerr << "ABORT: Unknown JSON type. This should not happen." << std::endl;
            std::terminate();
        }
    }
    return NULL;
...
constexpr const char* QuoteFieldsToString(QuoteFields& qtype) throw() {
    switch (qtype) {
        ...
        case QuoteFields::kAsNeeded:
            return "ASNEEDED";
        default: {
            std::cerr << "ABORT: Unknown quote fields. This should not happen." << std::endl;
            std::terminate();
        }
    }
    return NULL;
...
struct SelectRequest {
    ...
    SelectRequest(std::string expression,
                  CsvInputSerialization* csv_input,
    ...
    SelectRequest(std::string expression,
                  JsonInputSerialization* json_input,
                  CsvOutputSerialization* csv_output) {
        this->expr = expression;
...
struct SelectResult {
    ...
    SelectResult(long int bytes_scanned, long int bytes_processed, long int bytes_returned) {
...
using NotificationRecordsFunction = std::function<bool(std::list<NotificationRecord>)>;
...
struct Filter {
    ...
    operator bool() const {
        return and_operator ^ prefix ^ tag;
    }
}; // struct Filter
