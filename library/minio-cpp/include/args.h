...
struct SetBucketEncryptionArgs : public BucketArgs {
    ...
    SetBucketEncryptionArgs(SseConfig& sseconfig)
        : config(sseconfig) {
    }
    error::Error Validate();
...
struct SetBucketReplicationArgs : public BucketArgs {
    ...
    SetBucketReplicationArgs(ReplicationConfig& value)
        : config(value) {
    }
...
struct SetBucketLifecycleArgs : public BucketArgs {
    ...
    SetBucketLifecycleArgs(LifecycleConfig& value)
        : config(value) {
    }
...
inline constexpr unsigned int kDefaultExpirySeconds = (60 * 60 * 24 * 7); // 7 days
...
struct PostPolicy {
    ...
    operator bool() const {
        return !bucket.empty() && !expiration_;
    }
    ...
    error::Error AddStartsWithCondition(std::string element, std::string value) {
        ...
        if (element == "success_action_status" || element == "content-length-range" ||
            (utils::StartsWith(element, "x-amz-") && utils::StartsWith(element, "x-amz-meta-")))
        {
    ...
    error::Error FormData(std::map<std::string, std::string>& data,
        ...
        if (conditions_[eq_]["key"].empty() && conditions_[starts_with_]["key"].empty()) {
        ...
        if (lower_limit_ && upper_limit_) {
            conditions.push_back({"content-length-range", lower_limit_.Get(), upper_limit_.Get()});
        }
        ...
        std::string signature = signer::PostPresignV4(encoded_policy, secret_key, date, region);
...
private:
    ...
    static std::string getCredentialString(std::string access_key, utils::Time date, std::string region) {
        return access_key + "/" + date.ToSignerDate() + "/" + region + "/s3/aws4_request";
    }

    static bool isReservedElement(std::string element) {
        return element == "bucket" || element == "x-amz-algorithm" || element == "x-amz-credential" ||
               element == "x-amz-date" || element == "policy" || element == "x-amz-signature";
    }
}; // struct PostPolicy
} // namespace s3
} // namespace minio
