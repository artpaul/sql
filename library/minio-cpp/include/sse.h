...
public:
    SseCustomerKey(std::string_view key) {
        ...
        this->headers_.Add("X-Amz-Server-Side-Encryption-Customer-Algorithm", "AES256");
        ...
        this->copy_headers_.Add("X-Amz-Copy-Source-Server-Side-Encryption-Customer-Algorithm", "AES256");
        this->copy_headers_.Add("X-Amz-Copy-Source-Server-Side-Encryption-Customer-Key", b64key);
        this->copy_headers_.Add("X-Amz-Copy-Source-Server-Side-Encryption-Customer-Key-MD5", md5key);
    }
...
public:
    SseKms(std::string_view key, std::string_view context) {
        this->headers_.Add("X-Amz-Server-Side-Encryption-Aws-Kms-Key-Id", std::string(key));
        this->headers_.Add("X-Amz-Server-Side-Encryption", "aws:kms");
        if (!context.empty()) {
            this->headers_.Add("X-Amz-Server-Side-Encryption-Context", utils::Base64Encode(context));
        }
    }
