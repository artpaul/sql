...
constexpr const char* MethodToString(Method& method) throw() {
    switch (method) {
        ...
        case Method::kDelete:
            return "DELETE";
        default: {
            std::cerr << "ABORT: Unimplemented HTTP method. This should not happen." << std::endl;
            std::terminate();
        }
    }
    return NULL;
...
struct Request {
    ...
    Response Execute();
    operator bool() const {
        if (method < Method::kGet || method > Method::kDelete)
            return false;
        return url;
    }
...
struct Response {
    ...
    error::Error Error() {
        ...
        if (status_code && (status_code < 200 || status_code > 299)) {
            return error::Error("failed with HTTP status code " + std::to_string(status_code));
        }
