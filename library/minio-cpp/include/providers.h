...
public:
    StaticProvider(std::string access_key, std::string secret_key, std::string session_token = "") {
        this->creds_ = Credentials{error::SUCCESS, access_key, secret_key, session_token};
    }
...
public:
    AwsConfigProvider(std::string filename = "", std::string profile = "") {
        ...
        if (profile.empty()) {
            if (!utils::GetEnv(profile, "AWS_PROFILE"))
                profile = "default";
        }
        ...
        if (reader.ParseError() < 0) {
            ...
            this->creds_ = Credentials{error::SUCCESS, reader.Get(profile, "aws_access_key_id", ""),
...
public:
    MinioClientConfigProvider(std::string filename = "", std::string alias = "") {
        if (filename.empty())
            filename = utils::GetHomeDir() + "/.mc/config.json";

        if (alias.empty()) {
            if (!utils::GetEnv(alias, "MINIO_ALIAS"))
                alias = "s3";
        }
        ...
        if (json.contains("hosts")) {
            ...
            this->creds_ = Credentials{error::Error("invalid configuration in file " + filename)};
            return;
        }

        if (!aliases.contains(alias)) {
            this->creds_ = Credentials{error::Error(
                "alias " + alias + " not found in MinIO client configuration file " + filename)};
            return;
        }

        this->creds_ =
            Credentials{error::SUCCESS, aliases[alias]["accessKey"], aliases[alias]["secretKey"]};
    }
...
public:
    AssumeRoleProvider(http::Url sts_endpoint,
                       std::string access_key,
                       std::string secret_key,
        ...
        if (!role_arn.empty())
            map.Add("RoleArn", role_arn);
        if (!role_session_name.empty()) {
            map.Add("RoleSessionName", role_session_name);
        }
        if (!policy.empty())
            map.Add("Policy", policy);
        if (!external_id.empty())
            map.Add("ExternalId", external_id);
    ...
    Credentials Fetch() {
        ...
        signer::SignV4STS(method, sts_endpoint_.path, region_, headers, utils::Multimap(), access_key_,
                          secret_key_, content_sha256_, date);
...
public:
    WebIdentityClientGrantsProvider(JwtFunction jwtfunc,
                                    http::Url sts_endpoint,
                                    unsigned int duration_seconds = 0,
                                    std::string policy = "",
                                    std::string role_arn = "",
                                    std::string role_session_name = "") {
    ...
    unsigned int getDurationSeconds(unsigned int expiry) {
        if (duration_seconds_)
            expiry = duration_seconds_;
        if (expiry > MAX_DURATION_SECONDS)
            return MAX_DURATION_SECONDS;
        if (expiry == 0)
            return expiry;
        if (expiry < MIN_DURATION_SECONDS)
    ...
    Credentials Fetch() {
        ...
        if (!resp) {
            ...
            creds_ = Credentials::ParseXML(resp.body, IsWebIdentity() ? "AssumeRoleWithWebIdentityResult"
...
public:
    ClientGrantsProvider(JwtFunction jwtfunc,
                         http::Url sts_endpoint,
                         unsigned int duration_seconds = 0,
                         std::string policy = "",
                         std::string role_arn = "",
...
public:
    WebIdentityProvider(JwtFunction jwtfunc,
                        http::Url sts_endpoint,
                        unsigned int duration_seconds = 0,
                        std::string policy = "",
                        std::string role_arn = "",
...
private:
    ...
    Credentials fetch(http::Url url) {
        ...
        http::Response resp = req.Execute();
        if (!resp)
            return Credentials{resp.Error()};

        nlohmann::json json = nlohmann::json::parse(resp.body);
        std::string code = json.value("Code", "Success");
        if (code != "Success") {
            return Credentials{error::Error(url.String() + " failed with code " + code + " and message " +
                                            json.value("Message", ""))};
        }

        std::string expiration = json["Expiration"];
        return Credentials{error::SUCCESS, json["AccessKeyId"], json["SecretAccessKey"], json["Token"],
...
public:
    LdapIdentityProvider(http::Url sts_endpoint, std::string ldap_username, std::string ldap_password) {
        this->sts_endpoint_ = sts_endpoint;
        utils::Multimap map;
...
public:
    CertificateIdentityProvider(http::Url sts_endpoint,
                                std::string key_file,
